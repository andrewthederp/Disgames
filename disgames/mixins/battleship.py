...
class Battleships(commands.Cog):
	...
	async def on_reaction_add(self, reaction, userr):
		if userr.id in self.boards and not reaction.message.guild and reaction.message.author == self.bot.user and userr != self.bot.user:
			try:
				if reaction.message.embeds[0].title == 'Battleship':
					if str(reaction) == '1️⃣':
						embed = discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[userr.id][0]), color=discord.Color.blurple())
						await reaction.message.edit(embed=embed)
					elif str(reaction) == '2️⃣':
						embed = discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[userr.id][1]), color=discord.Color.red())
						await reaction.message.edit(embed=embed)
			except IndexError:
				pass
	...
	async def battleship(self, ctx, member:discord.Member):
		...
		self.boards[ctx.author.id] = [[['🌊' for _ in range(8)] for _ in range(8)],[['🌊' for _ in range(8)] for _ in range(8)]]
		self.boards[member.id] = [[['🌊' for _ in range(8)] for _ in range(8)],[['🌊' for _ in range(8)] for _ in range(8)]]
		embed = discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[ctx.author.id][0]), color=discord.Color.blurple())
		try:
		...
		for i, ship in enumerate(ships_copy):
			...
			await msg_1.edit(embed=discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[ctx.author.id][0]), color=discord.Color.blurple()))
			await ctx.author.send("Enter coordinates:")
			inp = await self.bot.wait_for('message', check=lambda m:m.author == ctx.author and m.guild is None)
			await ctx.author.send('up/down/left/right:')
			tru_dir = await self.bot.wait_for('message', check= lambda m: m.author == ctx.author and m.guild is None)
			if tru_dir.content.lower() not in ['up','down','left','right']:
				await ctx.author.send("Invalid syntax: Thats not a valid direction, try again", delete_after=7.5)
			...
			for num in range(len(ship)):
				try:
					...
					if x not in range(8) or y not in range(8):
						await ctx.author.send("Invalid syntax: Cant add the ships there, try again!", delete_after=7.5)
						ships_copy.insert(i, ship)
						self.boards[ctx.author.id][0] = brd
						continue
					if self.boards[ctx.author.id][0][x][y] != '🌊':
						await ctx.author.send("Invalid syntax: Cant have 2 ships overlap eachother, try again!", delete_after=7.5)
						ships_copy.insert(i, ship)
						self.boards[ctx.author.id][0] = brd
						continue
			...
			await msg_1.edit(embed=discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[ctx.author.id][0]), color=discord.Color.blurple()))
		msg_1 = await ctx.author.send(embed=discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[ctx.author.id][0]), color=discord.Color.blurple()))
		...
		for i, ship in enumerate(ships_copy):
			...
			inp = await self.bot.wait_for('message', check=lambda m:m.author == member and m.guild is None)
			await member.send('up/down/left/right:')
			tru_dir = await self.bot.wait_for('message', check= lambda m: m.author == member and m.guild is None)
			if tru_dir.content.lower() not in ['up','down','left','right']:
				await member.send("Invalid syntax: Thats not a valid direction, try again", delete_after=7.5)
				ships_copy.insert(i, ship)
			...
			for num in range(len(ship)):
				try:
					if tru_dir.content.lower() in ['down','right']:
						...
						y = ((int(inp.content[1])-1) - dct[direction][1]*num)
					if x < 0 or y < 0:
						await member.send("Invalid syntax: Cant add the ships there, try again!", delete_after=7.5)
						ships_copy.insert(i, ship)
						self.boards[member.id][0] = brd
						continue
					if self.boards[member.id][0][x][y] != '🌊':
						await member.send("Invalid syntax: Cant have 2 ships overlap eachother, try again!", delete_after=7.5)
						ships_copy.insert(i, ship)
						self.boards[member.id][0] = brd
						continue
			...
			await msg_2.edit(embed=discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[member.id][0]), color=discord.Color.blurple()))
		msg_2 = await member.send(embed=discord.Embed(title='Battleship', description=self.format_battleships_board(self.boards[member.id][0]), color=discord.Color.blurple()))
		await msg_2.add_reaction('1️⃣')
		await msg_2.add_reaction('2️⃣')
		while True:
			m = await turn.send("Enter coordinates to attack:")
			inp = await self.bot.wait_for('message', check= lambda m: m.author == turn and m.guild is None)
			...
			try:
				...
				continue
			if x not in range(8) or y not in range(8):
				await turn.send(f"Invalid Syntax: {x+1}{y+1} isn't a valid place on the board")
				continue
			if self.boards[other_turn.id][0][x][y] not in ['🌊','🔥']:
				await turn.send(f"You fired at {x+1}{y+1} and it was a hit!")
				await other_turn.send(f"{turn.display_name} fired at {x+1}{y+1} and it was a hit :pensive:")
			else:
				await turn.send(f"You fired at {x+1}{y+1} and missed :pensive:")
				await other_turn.send(f"{turn.display_name} fired at {x+1}{y+1} and missed!")
